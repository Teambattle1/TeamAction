...
import {
    ...
    Wand2, Zap, CheckCircle, XCircle, GripHorizontal, Navigation, AlertTriangle, QrCode, Loader2
} from 'lucide-react';
    ...
    return (
        <div className="fixed inset-0 z-[5000] bg-[#0f172a] text-white flex flex-row overflow-hidden font-sans animate-in fade-in">
            ...
            <div className="flex-1 relative flex flex-col bg-[#050505]">
                ...
                {/* Floating QR Scanner Button - ENABLED BY DEFAULT */}
                {showQRScanner && (
                    <div className="absolute z-[1100] bottom-8 right-8 pointer-events-auto">
                        <button
                            onClick={handleQRScanClick}
                            disabled={isQRScannerActive && !qrScannedValue}
                            className={`flex items-center gap-2 px-5 py-3 rounded-full transition-all cursor-pointer shadow-2xl font-bold uppercase tracking-widest text-sm ${
                                qrScannedValue
                            ...
                            title={isQRScannerActive ? 'Click to stop scanning' : 'Click to scan QR code for task activation'}
                            type="button"
                        >
                                ...
                                <div className="w-full max-w-md bg-slate-900 rounded-2xl shadow-2xl overflow-hidden border-2 border-indigo-500">
                                    ...
                                    <div className="bg-black relative h-80 flex items-center justify-center overflow-hidden">
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                            <div className="w-64 h-64 border-4 border-indigo-400/60 rounded-xl shadow-lg animate-pulse" />
                                        </div>
                                    ...
                                    {/* Status Messages */}
                                    {qrScannerError && (
                                        <div className="bg-red-500/20 border-t border-red-500 px-6 py-3">
                                            <p className="text-xs text-red-200 font-bold">{qrScannerError}</p>
                                        </div>
                                    )}
                                    ...
                                    {!qrScannedValue && !qrScannerError && (
                                        <div className="px-6 py-4 border-t border-slate-700 text-center">
                                            <p className="text-xs text-slate-400 font-bold">Scanning for QR codes...</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        {/* QR Scanned Toast */}
                        {qrScannedValue && !isQRScannerActive && (
                            <div className="absolute top-full mt-3 left-1/2 -translate-x-1/2 bg-green-600 text-white px-4 py-2 rounded-lg whitespace-nowrap text-xs font-bold animate-bounce">
                                ✓ QR Code: {qrScannedValue.substring(0, 20)}...
                            </div>
                        )}
                    </div>
                )}
